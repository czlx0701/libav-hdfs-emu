...
IFLAGS     := -I. -I$(SRC_PATH)
CPPFLAGS   := $(IFLAGS) $(CPPFLAGS) $(shell curl-config --cflags)
CFLAGS     += $(ECFLAGS) $(shell curl-config --cflags)
...
$(PROGS): %$(EXESUF): %.o $(FF_DEP_LIBS)
	$(LD) $(LDFLAGS) $(LDEXEFLAGS) $(LD_O) $(OBJS-$*) $(FF_EXTRALIBS) $(shell curl-config --static-libs) $(shell curl-config --libs)
